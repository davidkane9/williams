...
Next up are faculty. Our package provides a `faculty` data set, created by `create_faculty()`, with information on the `r nrow(faculty)` current members of the faculty. Using `create_faculty(complete = TRUE)`, the user can create a tibble which includes the time series of faculty members since 2000. 

Information scraped from the archived course catalogs is saved as `.txt` files in the `inst/extdata` directory. These files follow the naming convention of the course catalog from which they are derived: `faculty-<YEAR (YYYY)>-<YEAR + 1 (YYYY)>`. We include a `year` variable in the final tibble. For example, `year = 2016` means that the information comes from the 2016-2017 course catalog.  

Consider a portion of the `faculty-2000-2001.txt` file. We will use it as an example to demonstrate the data munging procedure for faculty: 
