...
We have records of faculty firstnames and lastnames. 
...
The longest name is `r faculty_details %>% select(year, full.name) %>% filter(min_rank(-nchar(full.name)) < 2) %>% as.character() %>% tail(1)` and the shortest name is `r faculty_details %>% select(year, full.name) %>% filter(min_rank(-nchar(full.name)) < 2) %>% as.character() %>% head(1)`.
