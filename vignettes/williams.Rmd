...
```{r}
faculty_details %>% select(year, gender) %>% count(year, gender) %>% mutate(percentage = n/sum(n)) %>% ggplot(aes(year, percentage, color = gender)) + geom_line()
```
...
```{r}
faculty_details %>% select(year, race) %>% count(year, race) %>% mutate(percentage = n/sum(n)) %>% ggplot(aes(year, percentage, color = race)) + geom_line()
```

##Undegraduate Degree

The `faculty` and `faculty_details` datasets provide information about faculty's undergraduate college degree, the conferring school, and the year the degree was received. 

The top 10 most common colleges that faculty attended are: 
...
The `faculty` and `faculty_details` datasets provide information about the highest degree earned by faculty, the conferring school, and the year the degree was received. 
...
```{r}
qplot(faculty$age[which(faculty$gender == "male")], geom="histogram", fill = I("cyan3"), color = I("black") )
```

```{r}
qplot(faculty$age[which(faculty$gender == "female")], geom="histogram", fill = I("firebrick3"), color = I("black") )
```
