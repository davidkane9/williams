...
```{r}
faculty.na <- filter(faculty_details, year == 2014)
faculty.na <- faculty.na[which(!is.na(faculty.na$department)), ]
grouped <-  group_by(faculty.na, department)
summary.df <- summarise(grouped, number_of_faculty = n())
summary.df <- summary.df[with(summary.df, order(-number_of_faculty)), ]
```

```{r, fig.margin = TRUE, fig.height=20, fig.cap="Faculty employed by department 2014 - 2015"}
p <-ggplot(summary.df, aes(department, number_of_faculty))
p +geom_bar(stat = "identity", fill = "purple") + theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.text=element_text(size=15),
        axis.title=element_text(size=25,face="bold")) + 
  labs(x = "Department", y = "Number of faculty") + coord_flip() 
```
...
Other's are more complicated. For example, consider "Bernadette Brooten, Croghan Bicentennial Visiting Professor in Biblical
...
Alas, other's are, still, more complicated. "Jennifer Austin, Assistant Professor of Romance Languages" is a case in point: it is obvious that she teaches Romance Languages, but does she teach French, Italian, or Spanish? These cases need to be handled explicitly.       


According to our findings, the top 10 departments in terms of faculty employed for the latest year we have data for (2014-2015) are: 

```{r}
summary.df[1:10, ]  %>% as.data.frame()
```
